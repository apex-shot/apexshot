...
Within that scope, the earlier high-level conclusion holds and is now more precise: Apexshot has implemented the Motion transform/timeline work and most of Shotbase’s **Appearance / Background** card-scene foundation. It has not implemented the remaining Shotbase image-Motion tools: Browser Chrome, Motion cursor overlay, Motion camera overlay, or image-Motion audio tracks. Watermark now has a user-image layer with size, inset, and XY controls; it does not yet replicate a Shotbase built-in catalog or byte-backed project resource. Border and the recovered Shadow controls are implemented. **Frame presets are now implemented**: an independent `MotionFrame` state (`framePresetId` contract), a compositor stage that re-fits the scene, per-preset MP4 output sizes, and a Frame inspector section — the aspect values are Apexshot decisions because the binary does not expose them. **Gradient presets are now live**: `selected_gradient_preset_index` is set by an 8-preset picker with wallpaper-style catalog UI; preset visuals are Apexshot's own. **Scene Shadows are now implemented**: an independent `MotionSceneShadow` state (`sceneShadowPresetId`/opacity/placement contract), Underlay/Overlay compositor passes shared by preview and export, and an inspector section — the six shading presets are Apexshot visuals because Shotbase's shadow assets are not recoverable.
...
| --- | --- | --- | --- |
| Appearance → Background | Fill cases `none`, `color`, `gradient`, `wallpaper`, `image`; padding, blur, noise; gradient-preset index; wallpaper/custom-image state. | `MotionBackgroundFillType` has the same five fill cases. The Appearance inspector provides fill selection, color and two-stop gradient editing, an 8-preset gradient strip (sets `selected_gradient_preset_index`; Apexshot's own preset visuals), selected wallpaper/image files, padding, background blur, noise, preview, and MP4 export rendering. | Partial-to-strong |
...
| Overlays → Scene Shadows | Preset ID, opacity, placement; overlay/underlay render-layer distinction; 14 numbered shipped shadow presets. | **Present.** `MotionSceneShadow`/`MotionSceneShadowPreset`/`MotionSceneShadowPlacement` state, a Scene Shadows inspector section (preset picker, opacity, Underlay/Overlay placement), and compositor passes shared by preview and MP4 export. Underlay draws between background and card, overlay above card and titles but below the watermark. The six shading presets are procedural Apexshot visuals because Shotbase's `Shadow-01`…`Shadow-14` assets are not recoverable. | Present (preset visuals are Apexshot decisions) |
| Frame | `framePresetId`; explicit `Frame` UI. Labels include Standard, Instagram, X (Twitter), and YouTube. | **Present.** `MotionFramePreset`/`MotionFrame` state, Frame inspector section, preview scene re-fit, and per-preset MP4 output dimensions (Instagram 1:1, X 1.91:1, YouTube 16:9; aspects are Apexshot decisions). | Present (aspect values are Apexshot decisions) |
...
The remaining confirmed Background gap is catalog parity. Apexshot Motion now reuses its bundled app-background catalog for Wallpapers, and its Gradient section now offers an 8-preset strip that sets the recovered `selectedGradientPresetIndex` contract (Apexshot's own preset visuals, since Shotbase's are not recoverable); custom two-color editing remains available and clears the preset selection. Image remains a user-chosen file.
...
**Implemented.** Apexshot now has an independent `MotionSceneShadow` layer matching Shotbase's `sceneShadowSnapshot` contract: preset id, opacity, and Underlay/Overlay placement, with a Scene Shadows section in the Motion Appearance inspector and compositor passes shared by preview and MP4 export — underlay between the background scene and the card, overlay above card and titles but below the watermark. Shotbase's `Shadow-01`…`Shadow-14` assets and their overlay/underlay images are not recoverable, so the shipped presets (Diagonal, Window, Top, Bottom, Vignette, Side) are procedural Apexshot visuals. An unset background fill renders as a black export scene, so the shadow is not visible there; the inspector states this. Apexshot also has a distinct user-image Watermark layer, but not Shotbase's byte-backed/catalog resource semantics. Its regular card drop shadow remains separate: it has no preset, no overlay/underlay placement, and no independent asset selection.
...
**Implemented.** Apexshot now has an independent `MotionFrame`/`MotionFramePreset` state (`framePresetId` contract), a Frame section in the Motion inspector, and a compositor stage that re-fits the scene into the largest centered rectangle of the preset's aspect — shared by preview, text hit-testing, and export. MP4 output dimensions switch per preset (Instagram 1920×1920, X 1920×1004, YouTube/Standard 1920×1080). The exact Shotbase aspect values, masks, and safe areas remain unrecovered, so the shipped aspects are Apexshot design decisions.
...
1. ~~Expand the bundled background/gradient preset library~~ **Done for gradients** (8-preset picker now sets `selected_gradient_preset_index`); wallpaper catalog reuse already shipped. Closer Shotbase catalog parity (55 wallpapers) remains optional.
2. Extend the compositing-layer contract for the remaining image-Motion tools: durable state, preview/export equivalence, resource ownership, and explicit z-order. The Shotbase snapshot names establish that Browser Chrome, Watermark, Scene Shadows, Cursor, and Camera are independent layers. ~~Frame~~ **Done** — its snapshot/state/stage/output-size pattern is the template.
3. ~~Add Frame as a deterministic image layer~~ **Done** (see Frame findings). Watermark's first user-image layer is implemented; catalog/resource ownership parity remains optional follow-up work.
4. ~~Add Scene Shadows as asset-backed overlay/underlay placement—not as an extension of card drop shadow.~~ **Done** (see Scene Shadows findings) with procedural Apexshot presets standing in for the unrecoverable asset catalog.
