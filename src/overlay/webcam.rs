...
pub(crate) struct WebcamPreview {
    ...
    stop: Arc<AtomicBool>,
    pipeline: Arc<Mutex<Option<gst::Element>>>,
}
...
impl Drop for WebcamPreview {
    fn drop(&mut self) {
        ...
        if let Some(pipeline) = self.pipeline.lock().ok().and_then(|mut p| p.take()) {
...
pub(crate) fn enumerate_webcam_devices() -> Vec<i32> {
    ...
    // UVC cameras often expose multiple /dev/videoN nodes with the same name
    // (metadata/IR/alternate format nodes). Match the C++ overlay behavior:
    // walk in numeric order and keep only the first usable node per camera name.
    ...
    std::thread::spawn(move || {
        while !stop_thread.load(Ordering::Relaxed) {
            ...
            if width > 0 && height > 0 && data.len() >= (width as usize * height as usize * 4) {
                ...
                // Cairo ARgb32 expects premultiplied BGRA on little-endian. Webcam frames are opaque.
