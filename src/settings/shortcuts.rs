...
use gtk4::{prelude::*, Align, Box as GtkBox, Grid, Label, Orientation, Image, Button};
...
pub fn build_shortcuts_section(config: &AppConfig) -> ShortcutSettingsWidgets {
    ...
    let create_row = |grid: &Grid, label: &str, current_val: &str, row: &mut i32, is_zebra: bool| -> Button {
        if is_zebra {
        ...
        btn.set_label(if current_val.is_empty() { "Record shortcut" } else { current_val });
    ...
    let toggle_icons_btn = create_row(&grid, "Toggle Desktop Icons:", &config.shortcut_toggle_desktop_icons, &mut row, false);
    let open_file_btn = create_row(&grid, "Open File:", &config.shortcut_open_file, &mut row, true);
    let open_clipboard_btn = create_row(&grid, "Open From Clipboard:", &config.shortcut_open_from_clipboard, &mut row, false);
    let pin_screen_btn = create_row(&grid, "Pin to the Screen:", &config.shortcut_pin_to_screen, &mut row, true);
    let restore_file_btn = create_row(&grid, "Restore Recently Closed File:", &config.shortcut_restore_recently_closed, &mut row, false);
    let toggle_overlays_btn = create_row(&grid, "Hide/Show Overlays:", &config.shortcut_toggle_overlays, &mut row, true);

    // --- Screenshots Section ---
    create_header(&grid, "Screenshots", "camera-photo-symbolic", &mut row);
    let capture_area_btn = create_row(&grid, "Capture Area:", &config.shortcut_capture_area, &mut row, false);
    let capture_prev_btn = create_row(&grid, "Capture Previous Area:", &config.shortcut_capture_previous_area, &mut row, true);
    let capture_fullscreen_btn = create_row(&grid, "Capture Full Screen:", &config.shortcut_capture_fullscreen, &mut row, false);
    let capture_window_btn = create_row(&grid, "Capture Window:", &config.shortcut_capture_window, &mut row, true);
