...
use gtk4::{
    prelude::*, Align, Box as GtkBox, Button, CenterBox, CheckButton, ColorButton, Grid, Image,
    Label, Orientation, Scale, Separator,
};
...
pub fn build_wallpaper_section(config: &AppConfig) -> WallpaperSettingsWidgets {
    ...
    row += 1;
    let wallpaper_dont_change_check =
        CheckButton::with_label("Don't change the wallpaper when switching spaces");
    ...
    let l_spacer2 = GtkBox::new(Orientation::Horizontal, 0);
    l_spacer2.set_hexpand(true);
    let r_spacer2 = GtkBox::new(Orientation::Horizontal, 0);
    r_spacer2.set_hexpand(true);
